...
### Background
I just want to give some background how I came across ulid and why I found it handy to use. I regularly find myself browsing through code from open source projects, even if I am not familiar with the languaje the projects are written, I always take a peek at the code. I have this obsesion to know how things work under the hood, which has helped me to solve many tricky issues operating open source software.  
...
# tsdb (time servies database) structure is stored.
...
```
Ever since I looked into that folder I wondered about the logic behind the design. The some day I was looking trought the prometheus code and I came across the term ulid. Off course I quickly googled the term and found out what it was, then the whole directory structure came to sense. I won't get into details of that as it is a subject matter that probably could get 2 or 3 blog posts itself. But I will get into details what ulid are and it might give developers some perspective to solve some problems in a clever way like promtetheus developers did.
...
But in plain words it is a 128 bit ID, represented in a string using Crockford's Base32 encoding. It is compossed by a timestam which is the fist 48bits and a secuence of random 80 bits giving a 26 character string.  
...
### Benefits
#### Lexicograpically sortable
This is the main benefit, making the IDs sortable through time. Not only that you can sort by the order of creatin but the ulid has the timestamp enconded in it when it was created, which probably might help optimize using the id to also store the creation time.  
...
#### UUID Compatible
Both UUID and ulid are represented as 128 bit, making them compatible when storing them at rest. This is an advantage to leverage existing methods to store UUIDs, for example some databases support UUID as a data type or they can be extended to support soting UUID as data type, which makes it convenient to reuse the UUID data types to store ulid.
...
This is an advantage and a disadvantage at the same time. Why? It depends on the use case. While it is great to have the timestamp embeded in the ID and handy for some use cases, when you need a pure random ID having something like the timestamp could be and anti pattern. We need to be careful with this feature as it might expose sensitive information in the ID so we need to be aware of the use case before using it.

#### No RFC
There are no standards defining ulids, this is just a spec that is hosted in github, which means that someone created the concept and sahed witht he world. While UUIDs are an industry standard and the have their own [rfc4122](https://www.ietf.org/rfc/rfc4122.txt), UUIDs can be used for interoperability between vendors. While ulid doesn't have an RFC which might be a problem when you need to interoperate with vendors.

#### Implementations might diverge
This is related to the previous disadvantage, since this is a spec and not backed by an industry standard, implemetation is subject to interpretation and some implementations might have slightly different behavioir than other. Off course having an RFC if not guarantee that implementations will be the same, in general RFC are topics already discussed by a group of people/vendors/companies making it a standard and making it easy for interoperate.

#### Still few tools to manipulate ulids
There are libraries for all major languajes, but ulid is still missing tools to manipulate ulids in the operating systems, all major operating systems like mac or linux already have some kind of tool for generating UUIDs like `uuidgen` or something similar. I havent found tools to generate ulids out of the box without having to install a programing laguaje and the library to generate the ulid.


### Use cases

This is not an exhaustive list of use cases, but some common use cases it can be used for. Most use cases are related to the objects/events need to be sorted by time, or ordered through time when precision doesn't require sub milisecond.
...
I wanted to point out a use case that catches my attention and probably would have saved some problems with race contitions when sending messages in an async messaging plattform and still have events that need to be ordered. In an distributed system sending messages/events to other systems and attaching a ulid to the message would help prevent or detect race conditions, won't guarantee that my messages arrived in order but at least I would know that I got a message older than a message that I already received. Helping detect and remediate the situation that might have generated a race condition. Well this is not a bulletproof approach but whould helped to detect some of this situations.

#### Example

As an example I would show how to generate a ulid in python but other languajes should be pretty similar. The the example I used `python-ulid` and `iPython` interpreter.

```
francisco@C02W60L0HV2V:~ $ ipython3
...
Now we can generat a ulid and extrat the timestamp from it
```
francisco@C02W60L0HV2V:~ $ ipython3
Python 3.7.5 (default, Nov  1 2019, 02:16:32)
Type 'copyright', 'credits' or 'license' for more information
...
```
And At last I will show how easy is to move swap form ulid to UUID and UUDI to ulid.
...
Whith this post I wanted to share something I lernt by diggin how things work under the hood, and I wanted to share ti because this might give other developers more tricks to solve issues in a different way or make their implementations much easier or simpler.  
I know ulid won't replace UUIDs because of the fact that both probably compliment each other and one covers use cases the other doesn't. As I mentioned a few times it would depend the use cases. I would still use UUIDs when I need them to be completely random and being random is a must or need. But I would use ulid when the use case is not sensitive to having the timestamp embeded in the ID I owuld choose to generate them with ulid generator, you never know when it would be useful and if the ulid was generated and then stored as UUID you can always recover the timestamp form the UUID if you sotred it as UUID. Since they are compatible there would be some benefits to generating the UUID from ulid.  
And as I already mentioned I would definitely use it for use cases where we need some object id ordered in time or sortable, when we need to also track the event in time I doule definitely choose ulid. 